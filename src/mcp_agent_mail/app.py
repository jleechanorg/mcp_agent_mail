...
async def _get_agent_by_name(name: str) -> Agent:
    ...
    async with get_session() as session:
        ...
        agent = result.scalars().first()
        if not agent:
            raise NoResultFound(f"Agent '{name}' not found. " f"Tip: Use register_agent to create a new agent.")
        return agent
...
def build_mcp_server() -> FastMCP:
    ...
    async def call_extended_tool(ctx: Context, tool_name: str, arguments: dict[str, Any]) -> Any:
        ...
        if tool_name not in EXTENDED_TOOLS:
            raise ValueError(
                f"Unknown extended tool: {tool_name}. " f"Use list_extended_tools to see available options."
            )

        tool_func = _EXTENDED_TOOL_REGISTRY.get(tool_name)
        if not tool_func:
            raise RuntimeError(f"Extended tool {tool_name} is not registered. " f"This is an internal server error.")
