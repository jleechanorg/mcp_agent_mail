        ...
        async with get_session() as sx:
            ...
            async def _route(name_list: list[str], kind: str) -> None:
                for raw in name_list:
                    ...
                    # Cross-project messaging: no link checks required
                    target_project_found: Optional[Project] = None
                    ...
                    if explicit_override and target_project_override is not None:
                        ...
                        if len(global_matches) == 1:
                            target_project_found, target_agent_found = global_matches[0]
                        elif len(global_matches) > 1:
                            # Ambiguous: multiple agents with same name in different projects
                    ...
                    if target_project_found and target_agent_found:
                        bucket = external.setdefault(
                            target_project_found.id or 0,
                            {"project": target_project_found, "to": [], "cc": [], "bcc": []},
                        )
                        bucket[kind].append(target_agent_found.name)
                        continue
            ...
            async def _route(name_list: list[str], kind: str) -> None:
                for raw in name_list:
                    ...
                    if target_project_found and target_agent_found:
                        ...
                        # Agent not found - add to unknown recipients for later validation
                        if explicit_override:
                            label = (
