...
@pytest.mark.asyncio
async def test_global_agent_discovery_and_messaging(isolated_env):
    ...
    async with Client(server) as client:
        ...
        req1 = await client.call_tool("request_contact", {
            "project_key": "/custom/project/alpha",
        ...
        assert req1.data.get("status") == "pending"

        await client.call_tool("respond_contact", {
            "project_key": "/custom/project/beta",
            "to_agent": "DevAgent",
            "from_agent": "Coordinator",
        ...
        req2 = await client.call_tool("request_contact", {
            "project_key": "/custom/project/alpha",
            "from_agent": "Coordinator",
            "to_agent": "DevAgent",
        ...
        assert req2.data.get("status") == "pending"
...
@pytest.mark.asyncio
async def test_agent_name_not_primary_key_scoped_per_project(isolated_env):
    ...
    async with Client(server) as client:
        # Create two projects
        await client.call_tool("ensure_project", {"human_key": "/test/project1"})
        await client.call_tool("ensure_project", {"human_key": "/test/project2"})

        # Register agent with same name in both projects - should succeed
        agent1 = await client.call_tool("register_agent", {
        ...
        agent2 = await client.call_tool("register_agent", {
        ...
        assert agent1.data["name"] == "CommonName"
        assert agent2.data["name"] == "CommonName"
        # They are different agents in different projects
        assert agent1.data.get("project_slug") != agent2.data.get("project_slug")


@pytest.mark.asyncio
async def test_custom_project_keys_independent_of_directory(isolated_env):
    ...
    async with Client(server) as client:
        ...
        for key in custom_projects:
            result = await client.call_tool("ensure_project", {"human_key": key})
            assert result.data["human_key"] == key

            # Register an agent in each custom project
            agent = await client.call_tool("register_agent", {
                "project_key": key,
...
@pytest.mark.asyncio
async def test_cross_project_reply_with_case_insensitive_lookup(isolated_env):
    ...
    async with Client(server) as client:
        # Setup two projects
        await client.call_tool("ensure_project", {"human_key": "/projectX"})
        await client.call_tool("ensure_project", {"human_key": "/projectY"})
        ...
        await client.call_tool("request_contact", {
            "project_key": "/projectX",
        ...
        original = await client.call_tool("send_message", {
            "project_key": "/projectX",
        ...
        message_id = original.data["message_id"]
        thread_id = original.data["thread_id"]

        # Bob replies using LOWERCASE name (testing case-insensitive lookup)
        reply = await client.call_tool("reply_message", {
            "project_key": "/projectY",
        ...
        assert reply.data["thread_id"] == thread_id

        # Verify Alice received the reply (case-insensitive should have worked)
        alice_inbox = await client.call_tool("fetch_inbox", {
            "project_key": "/projectX",
            "agent_name": "AliceAgent"
        })

        reply_msg = next((m for m in alice_inbox.data["messages"] if m["id"] == reply.data["message_id"]), None)
